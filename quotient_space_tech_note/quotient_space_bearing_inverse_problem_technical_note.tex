...
Physically, nuisance variables are typically constrained to compact ranges (bounded amplitude scaling, bounded phase or speed perturbation, bounded offset). This compactness of \(\EtaSpace\) is used in Section~\ref{sec:quotient_formulation} to establish boundedness of the nominal functional manifold.
...
Let \(\mathcal{T}\) denote the physically motivated family of nuisance transformations introduced in Section~\ref{sec:observation_model}:
amplitude scaling, phase shift, speed perturbation, offset, and additive noise.

For the formal quotient argument, we let \(\Gnuis\) denote the transformation group generated by \(\mathcal{T}\), together with the identity transformation and the inverses of its elements.
This is a closure construction: even if the physically motivated transformations in \(\mathcal{T}\) are sampled only over bounded operational ranges, the group generated by transformations of the same type is mathematically well defined.

The formal quotient-space construction is therefore stated in terms of the group action of \(\Gnuis\) on \(\X\).
The compactness argument below does not require the full group \(\Gnuis\) to be compact.
It uses only the compact nuisance parameter set \(\EtaSpace\) sampled in the Monte Carlo construction.
Thus, the mathematical quotient is defined using an honest group action, while the experiment samples a compact physically admissible subset of nuisance configurations.
...
Suppose the nuisance parameter space \(\EtaSpace\) is compact, and the generative map
\[
\eta \mapsto F(\theta_{\mathrm N},\eta)
\]
viewed as a map from \(\EtaSpace\) into the space of fixed-length observation windows equipped with the uniform, or sup-norm, topology, is uniformly bounded and equicontinuous over \(\EtaSpace\).
Then the nominal nuisance orbit
\[
...
\subset \X
\]
is relatively compact: its closure \(\overline{\mathcal{O}_{\mathrm N}}\) is a compact subset of \(\X\).
\end{proposition}
...
\]
is uniformly bounded and equicontinuous in the relevant sup-norm topology.
By the Arzel\`a--Ascoli theorem, this family is relatively compact in the sup-norm topology.
Therefore its closure \(\overline{\mathcal{O}_{\mathrm N}}\) is compact.

In the bearing instantiation of Section~\ref{subsec:bearing_instantiation}, \(\EtaSpace\) is a product of intervals and is therefore connected.
If the map \(\eta\mapsto F(\theta_{\mathrm N},\eta)\) is continuous, then its image \(\mathcal{O}_{\mathrm N}\) is connected.
\end{proof}

\begin{corollary}[Boundedness of the nominal quotient structure]
\label{cor:bounded_manifold}
Assume the hypotheses of Proposition~\ref{prop:compactness}.
Since the quotient projection \(\pi:\X\to\X/\Gnuis\) is continuous, the image
\[
\pi\left(\overline{\mathcal{O}_{\mathrm N}}\right)
\]
is compact in \(\X/\Gnuis\).
Moreover, in the connected setting described above, \(\pi(\mathcal{O}_{\mathrm N})\) is connected.
Since \(\Mnominal=\pi(\mathcal{O}_{\mathrm N})\) by Definition~\ref{def:nominal_quotient_manifold}, the nominal quotient manifold is contained in a compact quotient structure.
This provides the geometric justification for describing \(\Mnominal\) as a bounded, connected functional structure, and operationally as a functional manifold in the sense clarified below.
\end{corollary}

\begin{remark}[On the use of the term ``manifold'']
\label{rem:manifold_terminology}
Proposition~\ref{prop:compactness} and Corollary~\ref{cor:bounded_manifold} establish compactness and connectedness properties of the nominal structure under explicit assumptions, but they do not establish a smooth or topological manifold structure in the differential-geometric sense, such as local Euclidean charts, well-defined dimension, or curvature.
The term \emph{functional manifold} is used here, consistently with the companion report on manifold saturation~\cite{disanti2026} and with common usage in the representation-learning literature under the manifold hypothesis~\cite{bronstein2021}, to denote a bounded, connected region of the quotient space generated by a fixed latent regime under nuisance variability.
A fully differential-geometric treatment is left to future work.
\end{remark}

\begin{proposition}[Quotient factorization of diagnostic maps]
\label{prop:quotient_diagnosis}
Assume that \(\Gnuis\) acts on \(\X\), that the action preserves the latent physical state, and that a diagnostic map \(h:\X\to\mathcal{Y}\) depends only on that latent physical state.
Then \(h\) is constant on \(\Gnuis\)-orbits and therefore factors through the quotient projection \(\pi:\X\to\X/\Gnuis\).
That is, there exists a map \(\bar h:\X/\Gnuis\to\mathcal{Y}\) such that
\[
h = \bar h \circ \pi.
\]
\end{proposition}
...
\begin{proof}
Let \(x_1,x_2\in\X\) lie in the same \(\Gnuis\)-orbit.
Then \(x_2=g\cdot x_1\) for some \(g\in\Gnuis\).
By assumption, the action of \(g\) preserves the latent physical state, and the diagnostic map \(h\) depends only on that state.
Hence \(h(x_1)=h(x_2)\).
Therefore \(h\) is constant on equivalence classes.
By the universal property of quotient maps, \(h\) factors through \(\pi\), so there exists \(\bar h:\X/\Gnuis\to\mathcal{Y}\) such that \(h=\bar h\circ\pi\).
\end{proof}
...
%------------------------------------------------------------
\label{subsec:bearing_instantiation}
...
The reconstruction residual \(\residual(x)\) is treated as a computable proxy, monotonically related to a notion of distance to the nominal quotient manifold:
\begin{equation}
\residual(x) \approx d([x],\Mnominal).
\label{eq:distance_proxy}
\end{equation}
This relation is not derived from the autoencoder's training objective: minimizing mean-squared reconstruction error does not, by itself, guarantee that \(\residual\) is isometric, or even monotonic, with respect to any particular metric on the quotient space.
Assumption~\ref{ass:residual_proxy} should be read as an operational hypothesis, evaluated empirically in Section~\ref{sec:synthetic_validation}, not as a property established by construction.
\end{assumption}
...
As anticipated in Section~\ref{sec:quotient_formulation}, the formal quotient construction uses an honest group action \(\Gnuis\), while any empirical instantiation---synthetic or field-collected---can only sample a compact, physically admissible subset of that action.
In real machines, nuisance effects may not even compose exactly within \(\mathcal{T}\) itself, so the closure that defines \(\Gnuis\) should be read as a mathematical idealization of physical nuisance mechanisms rather than as an empirically verified algebraic property.
