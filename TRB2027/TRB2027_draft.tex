...
Transportation operators increasingly face the challenge of deploying condition-monitoring
systems across large fleets of heterogeneous assets without fault labels, asset-specific
representation learning, or lengthy calibration campaigns. We propose a day-one deployment
method that separates what is shared across the fleet from what must remain local to each
physical asset. A single canonical nominal representation is trained once, offline, using
simulated healthy observations and is subsequently transferred unchanged to field deployment.
Each asset then estimates only its own operational radius from a short stream of unlabeled local
observations, without modifying or retraining the shared representation.

The method is evaluated on four Case Western Reserve University bearing operating conditions and
compared with an ideal but non-scalable setting in which a separate detector is trained and
calibrated using the complete healthy dataset of each condition. Asset-specific autoencoders
achieve 100\% detection across the evaluated fault families. The proposed shared
simulator-trained model, using only a 100-window local warm-up, achieves an average healthy
false-alarm rate of $3.09\%$, ball-fault detection of $99.69\%$, and 100\% detection of
inner- and outer-race faults. The local operational radius converges after
$51.0\pm12.6$ windows. In contrast, direct transfer of the absolute simulator threshold produces
100\% false alarms, demonstrating that the transferable object is the canonical nominal
representation rather than its absolute residual scale.

The resulting deployment primitive consists of one shared representation, independent local
operational-radius convergence, and lightweight edge inference. Such a representation could be
developed and maintained by an original equipment manufacturer and distributed fleet-wide,
while operators perform only short, unsupervised local calibration during installation.
...
Although machine-learning research has produced increasingly capable anomaly detectors, the
deployment problem remains largely unresolved: a detector that performs well for one asset under
one calibrated operating condition does not automatically scale to hundreds or thousands of
independently installed units.
...
between assets. Fourth, training, calibrating, validating, and maintaining a separate
representation model for every physical unit creates an operational and organizational burden
that grows directly with fleet size.
...
be trained and calibrated for each one. The central question is then:

\begin{quote}
\emph{How much detection performance is lost when many asset-specific models are replaced by
one simulator-trained representation and a few seconds of local calibration?}
\end{quote}

We answer this question using four Case Western Reserve University bearing operating conditions.
The asset-specific reference methods are trained and calibrated using the complete healthy dataset
of each condition. In contrast, the proposed method uses no field observation for representation
learning and estimates its local operational radius from only the first 100 healthy windows,
corresponding to approximately ten seconds of signal at the evaluated window duration.
...
inner- and outer-race faults. The operational radius converges after $51.0\pm12.6$ windows on
average. Direct transfer of the absolute simulator threshold, however, produces 100\% healthy
false alarms, confirming that the representation transfers while its absolute score scale does
not.
...
The methodology is intentionally independent of asset type and sensing modality. It requires
only a nominal representation producing a scalar anomaly score, repeated observations from each
...
A compact geometric rationale is provided in Appendix~\ref{sec:theory}.
...
shared nominal coverage, independent operational-radius convergence, and local empirical
calibration, without fault labels or per-asset retraining.

The methodology is intentionally independent of both sensing modality and asset type. The present manuscript validates the approach on the CWRU bearing benchmark because it provides
controlled nominal observations, multiple fault modes, and several operating conditions.
Bearings should therefore be interpreted as the first validation domain rather than as a
...
than as a limitation of the proposed deployment method. Bearings
...
windows reserved for validation. A separate synthetic test set contains 750 windows per regime.
...
The experiments address two distinct questions. First, we establish the performance attainable
under an ideal but operationally non-scalable condition in which a separate detector is trained
and calibrated using the complete healthy dataset available for each CWRU operating condition.
Second, we evaluate whether the proposed simulator-trained shared representation, combined only
with a short online calibration stream, can approach that ideal performance without any
field-model retraining.

The CWRU evaluation includes four operating conditions corresponding to 1730, 1750, 1772, and
1797\,RPM. Across these conditions, the experiments contain 1,414 healthy windows, 1,621
ball-fault windows, 1,621 inner-race-fault windows, and 2,846 outer-race-fault windows. All
windows contain 1,200 samples and are processed identically across detectors.
...
The final two columns are reserved for repeated-run stability measurements.}
...
Figure~\ref{fig:tau-convergence} shows the sequential estimate of the local p98 operational
radius during the 100-window warm-up. Each condition converges to a distinct radius despite using
the same fixed representation. The final online estimates are close to their complete-data
field-oracle counterparts: 0.1984 versus 0.2005 at 1730\,RPM, 0.1759 versus 0.1786 at
1750\,RPM, 0.2072 versus 0.1997 at 1772\,RPM, and 0.2364 versus 0.2474 at 1797\,RPM.
...
\centering
\texttt{[PLACEHOLDER: operational-radius trajectories for 1730, 1750, 1772, and
1797\,RPM, including the complete-data field-oracle radius and detected convergence index.]}
\end{minipage}}
\caption{Online operational-radius convergence from the first 100 unlabeled healthy windows.
The shared representation remains unchanged; only the local empirical p98 radius is estimated.
Observed convergence occurs between 38 and 65 windows.}
\label{fig:tau-convergence}
\end{figure}

The convergence result is operationally relevant because the complete healthy distribution is not
available at the start of a real deployment. The asset-specific and field-oracle references assume
that all healthy data have already been collected. The proposed procedure instead constructs the
local acceptance boundary incrementally and approaches the complete-data radius within a small
fraction of the available nominal sequence.

The present experiment assumes that the initial warm-up observations are representative of
nominal operation. Detecting an asset that is already degraded at installation, or determining
that no stable nominal radius can be established, is outside the current scope. The convergence
dynamics themselves may provide evidence of an invalid initialization state and are identified as
...
\cite{disanti2026blueprints}, where diagnosis was interpreted as recovering a compact nominal
...
\bibitem{disanti2026quotient}
E. Di Santi.
\newblock \emph{Anomaly Detection as an Inverse Problem on Quotient Spaces, 
Equivalence Relations and Functional Manifolds as a General Diagnostic Framework}.
\newblock Technical report, University of Colorado Boulder, 2026.

\bibitem{disanti2026inverse}
E. Di Santi.
...
\bibitem{disanti2026blueprints}
E. Di Santi.
\newblock \emph{The Blueprints of Intelligence: A Functional-Topological Foundation for Perception and Representation}.
\newblock arXiv:2512.05089, 2025.
